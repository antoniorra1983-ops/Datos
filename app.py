...
Versión INTEGRAL Y RESTAURADA (v115 + MODULO ECO-DRIVING):
- FÍSICA: Integrador Euler Temporal (dt=1s). Escudo Aerodinámico y ATO Coasting.
- TERMODINÁMICA VIRTUAL: Flujo Nodal AC/DC. Balance Nodal Min-Function. Regen a 5km.
- GESTIÓN: Cortes y Acoples operando intactos. Motor de Fechas estable.
- MEJORA V115: Corrección de "Síndrome de Referencia Circular" en el cálculo del reóstato.
  Inyección de Auxiliares Dinámicos (Clima y Ocupación) y función Piecewise para Headway.
  Agregados paneles de Auditoría para SERs y SEAT calculando el IDE Global Real
  basado en el "Km Total" (Km Comercial + Km Vacío).
- NUEVO (MODULO ECO-DRIVING): Algoritmo de optimización cinemática con búsqueda binaria
  para maximizar deriva (coasting) respetando tiempos comerciales. Formato estricto de tablas.
"""
...
# --- NUEVO: PERFILES DE AUXILIARES DINÁMICOS ---
_AUX_HVAC_HORA = {
...
FLOTA = {
    "XT-100": {
        ...
        "aux_kw"       : 46.0      # V114: Restaurado al 100% nominal (dinámico)
    },
    "XT-M": {
        "tara_t"       : 95.0, 
        "m_iner_t"     : 8.0, 
        ...
        "aux_kw"       : 55.0      # V114: Restaurado al 100% nominal (dinámico)
    },
    "SFE": {
        "tara_t"       : 141.0, 
        "m_iner_t"     : 11.2, 
        ...
        "aux_kw"       : 190.0     # V114: Restaurado al 100% nominal (dinámico)
    },
}
...
def parse_time_to_mins(val):
    ...
    if m:
        h = int(m.group(1))
        m_min = int(m.group(2))
        s_sec = int(m.group(3)) if m.group(3) else 0
...
# =============================================================================
# 6. FÍSICA TERMODINÁMICA Y LOAD FLOW 
# =============================================================================
def simular_tramo_termodinamico(tipo_tren, doble, km_ini, km_fin, via_op, pct_trac, use_rm, use_pend, nodos=None, pax_dict=None, pax_abordo=0, v_consigna_override=None, maniobra=None, estacion_anio="primavera", t_ini_mins=0.0):
    ...
    for i in range(len(paradas_km)-1):
        ...
        while dist_recorrida < dist_total_tramo:
            ...
            pax_mid = get_pax_at_km(pax_dict, km_actual, via_op, pax_abordo)
    ...
    # V111 FIX: Dwell Estático (Baseline Planning Mode sin ruido THDR)
    ...
    aux_kw_dwell = calcular_aux_dinamico(
        f['aux_kw'] * (2 if doble else 1), hora_media_dwell,
    ...
    # V114 FIX: El reóstato se calcula fuera como reg_panto_max - reg_util
...
def distribuir_potencia_sers_kw(p_kw, km_punto, active_sers):
    ...
    for i in range(len(sers_sorted)-1):
        ...
        if s1[0] <= km_punto <= s2[0]:
            ...
            d1 = km_punto - s1[0]
            d2 = s2[0] - km_punto
            p1 = p_kw * (d2 / dist_total)
            p2 = p_kw * (d1 / dist_total)
            return {s1[1]: p1, s2[1]: p2}
...
@st.cache_data(show_spinner="Integrando Termodinámica de Flota (Euler)...")
def calcular_termodinamica_flota_v111(df_dia, pct_trac, use_pend, use_rm, use_regen, pct_headway_clasico, estacion_anio="primavera"):
    df_e = df_dia.copy()
    ...
    def _wrapper_energia(r):
        ...
        if not use_regen: 
            reg_util = 0.0
        elif pct_headway_clasico and r.name in pct_headway_clasico: 
            reg_util = reg_panto_max * pct_headway_clasico[r.name]
        else:
            reg_util = reg_panto_max

        # V114 BUG FIX: Reóstato = Max Energía Eléctrica que sube al pantógrafo - Energía aceptada por la red
...
def precalcular_red_electrica_v111(df_dia, pct_trac, use_rm, estacion_anio="primavera"):
    ...
    for via_ in [1, 2]:
        ...
        for tr in trains_data:
            t_start = max(t_min, tr['t_ini'])
            t_end = min(t_max, tr['t_fin'])
            ...
            for i in range(idx_start, idx_end):
                ...
                # Dinámica auxiliar para Balance Nodal
                hora_tick = m / 60.0
                p_aux_kw = calcular_aux_dinamico(
                    f['aux_kw'] * n_uni, hora_tick, tr['pax_abordo'],
                    f.get('cap_max', 398) * n_uni, estacion_anio, state
                )
                ...
                if state in ("BRAKE", "BRAKE_STATION", "BRAKE_OVERSPEED"):
                    ...
                    f_req_freno = max(0.0, masa_kg * a_freno_op - f_davis - f_pend)
                    f_disp_freno = min(f['f_freno_max_kn']*1000*n_uni, (f.get('p_freno_max_kw', f['p_max_kw']*1.2)*1000*n_uni)/max(0.1, v_ms)) if v_kmh >= f['v_freno_min'] else 0.0
                    f_regen_real = min(f_req_freno, f_disp_freno)
                    p_mech_kw = (f_regen_real * v_ms) / 1000.0
        ...
        for i in range(len(time_steps)):
            braking = braking_by_idx[i]
            accel = accel_by_idx[i]
            ...
            for b_idx, b_pos, p_gen in braking:
                ...
                if d <= LAMBDA_KM * 2:
                    eta_dist = ETA_MAX * np.exp(-d / LAMBDA_KM)
    ...
    try:
        ...
        def _extract_nodos(row):
            ...
            for c in lle_cols:
                idx = lle_est.get(c)
                val = _safe_get(row, c)
                if idx is not None and pd.notna(val):
                    nodos_temp.append((val, KM_ACUM[idx]))
            for c in sal_cols:
                idx = sal_est.get(c)
                val = _safe_get(row, c)
                if idx is not None and pd.notna(val):
...
def cargar_pax(data, fname, via_param=1):
    try:
        ...
        if df.empty:
            st.error(f"No se encontraron filas con horas de origen válidas en {fname}.")
            return pd.DataFrame()
...
def match_pax(row, df_pax):
    ...
    if pd.notna(t_i):
        ...
        if best_match['diff'] <= 15: 
            ...
            return pax_est_d, _to_int(best_match.get('CargaMax', 0)), mins_to_time_str(best_match.get('t_ini_p')), str(best.get('Nro_THDR', '')), best_match.name
...
# =============================================================================
# 8. MOTOR DE OPTIMIZACIÓN CINEMÁTICA Y ECO-CONDUCCIÓN (NUEVO V115)
# =============================================================================
def calcular_perfil_eco_driving(tipo_tren, km_ini, km_fin, via_op, pct_trac, t_objetivo_s):
    """
    Busca mediante iteración binaria el punto óptimo de corte de tracción (Coasting)
    para cumplir exactamente con t_objetivo_s minimizando la energía.
    Retorna un DataFrame con columnas separadas para Fecha y Hora y datos físicos.
    """
    f = FLOTA.get(tipo_tren, FLOTA["XT-100"])
    dist_total_m = abs(km_fin - km_ini) * 1000.0
    if dist_total_m < 10.0: return pd.DataFrame()
    
    dt = 1.0
    masa_kg = (f['tara_t'] + f['m_iner_t']) * 1000 
    
    # Parámetros físicos límite
    f_trac_max = f['f_trac_max_kn'] * 1000 * (pct_trac / 100.0)
    p_max_trac = f['p_max_kw'] * 1000 * (pct_trac / 100.0)
    a_freno_op = f['a_freno_ms2'] * 0.9
    
    def _simular_con_corte(dist_corte_m):
        t_acum = 0.0
        v_ms = 0.0
        dist_recorrida = 0.0
        trc_acum = 0.0
        
        # Guardaremos el perfil para retornarlo si es el óptimo
        perfil = []
        
        while dist_recorrida < dist_total_m:
            dist_restante = dist_total_m - dist_recorrida
            if dist_restante < 0.1: break
            
            km_actual = km_ini + (dist_recorrida/1000.0) if via_op == 1 else km_ini - (dist_recorrida/1000.0)
            v_cons_kmh = vel_at_km(km_actual, via_op, use_rm=False)
            v_kmh = v_ms * 3.6
            ...
            if dist_restante <= d_freno_req + (v_ms * dt * 1.2):
                estado = "BRAKE"
                a_net = -a_freno_op
            elif dist_recorrida >= dist_corte_m:
                estado = "COAST"
                a_net = -f_davis / masa_kg
            else:
                if v_kmh < v_cons_kmh - 1.0:
                    estado = "ACCEL"
                    f_disp = min(f_trac_max, p_max_trac / max(0.1, v_ms))
                    f_motor = f_disp
                    a_net = (f_motor - f_davis) / masa_kg
                else:
                    estado = "CRUISE"
                    f_motor = f_davis
                    a_net = 0.0
            ...
            if estado == "ACCEL" and v_new * 3.6 > v_cons_kmh: v_new = v_cons_kmh / 3.6
            ...
            if f_motor > 0:
                ...
                trc_acum += ((f_motor * step_m) / 3_600_000.0) / eta_din
                
            perfil.append({
                "Distancia_m": dist_recorrida,
                "Velocidad_kmh": v_kmh,
                "Estado": estado,
                "Consumo_Traccion_kWh": trc_acum,
                "Tiempo_s": t_acum
            })
            ...
            # Escape de seguridad si el tren se detiene por inercia antes de llegar
            if estado == "COAST" and v_ms < 0.5 and dist_restante > 10.0:
                return float('inf'), []
                
        return t_acum, perfil

    # Búsqueda Binaria
    corte_min_m = 10.0
    corte_max_m = dist_total_m
    mejor_perfil = []
    
    t_min_posible, _ = _simular_con_corte(corte_max_m)
    if t_objetivo_s < t_min_posible:
        # Imposible cumplir, devolvemos el perfil más rápido
        _, mejor_perfil = _simular_con_corte(corte_max_m)
    else:
        for _ in range(15):  # Iteraciones binarias
            corte_mid = (corte_min_m + corte_max_m) / 2.0
            t_mid, perf_mid = _simular_con_corte(corte_mid)
            mejor_perfil = perf_mid
            
            if t_mid == float('inf') or t_mid > t_objetivo_s:
                corte_min_m = corte_mid  # Necesitamos traccionar más
            else:
                corte_max_m = corte_mid  # Podemos derivar antes

    # Formateo estricto del DataFrame de salida
    if mejor_perfil:
        df_eco = pd.DataFrame(mejor_perfil)
        # Mandato Estricto: Fecha y Hora en columnas separadas
        now = datetime.now()
        df_eco.insert(0, "Fecha", now.strftime("%Y-%m-%d"))
        df_eco.insert(1, "Hora", df_eco["Tiempo_s"].apply(lambda s: (now + timedelta(seconds=s)).strftime("%H:%M:%S")))
        return df_eco
...
# =============================================================================
# CACHÉ Y CARGADORES DE STREAMLIT (UI)
# =============================================================================
...
with st.sidebar:
    ...
    n_cortes_v1 = st.slider("Doble a Simple en El Belloto (V1, PU-LI)", 0, 20, 0, help="Desacopla en horas valle (~10:00 y ~20:30)")
    n_cortes_pu_sa_v1 = st.slider("Doble a Simple en El Belloto (V1, PU-SA)", 0, 20, 0, help="Desacopla trenes cortos en BTO")
    n_acoples_v2 = st.slider("Simple a Doble en El Belloto (V2)", 0, 20, 0, help="Acopla previo a horas punta (~06:30 y ~17:30)")
    n_cortes_sa_v1 = st.slider("Doble a Simple en S. Aldea (V1)", 0, 20, 0, help="Desacopla en S. Aldea")
    n_acoples_sa_v2 = st.slider("Simple a Doble en S. Aldea (V2)", 0, 20, 0, help="Acopla en S. Aldea")

    st.divider()
    st.subheader("⚙️ Parámetros de Simulación")
    use_rm        = st.checkbox("🚦 Velocidades RM", value=False)
    pct_trac      = st.slider("⚙️ % Tracción Nominal", min_value=30, max_value=100, value=75, step=5)
    use_pend      = st.toggle("⛰️ Modelo Físico de Pendientes", value=True)
    use_regen     = st.toggle("⚡ Activar Regeneración", value=True)
    tipo_regen    = st.radio("Modelo de Regeneración", ["Físico (Load Flow / Squeeze Control)", "Probabilístico (Headway Clásico)"])
    
    st.divider()
    st.subheader("🌡️ Perfil de Climatización")
    st.caption("Ajusta la carga térmica del aire acondicionado (HVAC) según el mes del año.")
    ...
    active_ser_names = st.multiselect("SERs Activas (Simular Falla)", all_ser_names, default=all_ser_names)
    active_sers = [s for s in SER_DATA if s[1] in active_ser_names]
    if not active_sers:
        st.warning("⚠️ Debe haber al menos 1 SER activa. Se ha reactivado SER PO por seguridad.")
        active_sers = [SER_DATA[0]]
...
b1 = _all_blobs(f_v1, "gh_blobs_v1")
b2 = _all_blobs(f_v2, "gh_blobs_v2")
bx1 = _all_blobs(f_px1, "gh_blobs_px1")
bx2 = _all_blobs(f_px2, "gh_blobs_px2")
    ...
    if len(dfs_to_concat) > 0:
        ...
        def calc_tren_km_real(row):
            ...
            if man in ['CORTE_BTO', 'ACOPLE_BTO', 'CORTE_PU_SA_BTO']:
                km_man = KM_ACUM[14] 
                if k_s <= km_man <= k_e:
                    km_doble = abs(km_man - k_s)
                    km_simple = abs(k_e - km_man)
                    return (km_doble * 2.0) + (km_simple * 1.0)
        ...
        df_all['tren_km'] = df_all.apply(calc_tren_km_real, axis=1)
...
# =============================================================================
# DIAGRAMA ESQUEMÁTICO UNIVERSAL
# =============================================================================
def draw_diagram(df_act_plot, ser_accum_plot, seat_accum_plot, hora_str, titulo_extra="", active_sers_list=SER_DATA):
    W        = 1200
    KM_SCALE = W / KM_TOTAL
    ...
    for idx, row in df_act_plot.iterrows():
        ...
        if tipo == 'SFE':
            xt_lbl = f"SFE-{motriz}" if motriz else "SFE"
        elif tipo == 'XT-M':
            xt_lbl = f"XTM-{motriz}" if motriz else "XTM"
        else:
            xt_lbl = f"XT-{motriz}" if motriz else "XT"
...
# =============================================================================
# PANTALLA PRINCIPAL (MAPA OPERATIVO Y DASHBOARD)
# =============================================================================
st.title("🗺️ Sistema Avanzado MERVAL - Operación y Simulación")

def render_planificador():
    st.subheader("🔮 Planificador de Oferta y Escenarios (Simulación Sintética)")
    st.markdown("Diseña un día operativo desde cero. El motor calculará el consumo energético canónico y proyectará la demanda sin necesidad de cargar un archivo THDR real.")
    ...
    with col_p1:
        ...
        st.info(f"**Pax promedio por viaje asumido:** {pax_promedio_viaje} pax\n*(Basado en perfiles estadísticos históricos de MERVAL para día {tipo_dia_plan.lower()})*")
        
    with col_p2:
        st.markdown("**Matriz de Servicios (Timetable Sintético)**")
        if 'df_plan' not in st.session_state:
            st.session_state['df_plan'] = pd.DataFrame([
                ...
                {"Ruta": "PU-SA", "Flota": "SFE", "Configuración": "Simple", "Cantidad": 20},
                {"Ruta": "SA-PU", "Flota": "SFE", "Configuración": "Simple", "Cantidad": 20},
            ])
            
        rutas_disp = ["PU-LI", "LI-PU", "PU-SA", "SA-PU", "PU-BTO", "BTO-PU"]
        flotas_disp = ["XT-100", "XT-M", "SFE"]
        config_disp = ["Simple", "Doble"]
        
        df_plan_edit = st.data_editor(
            st.session_state['df_plan'],
            column_config={
                "Ruta": st.column_config.SelectboxColumn("Ruta", options=rutas_disp, required=True),
                "Flota": st.column_config.SelectboxColumn("Familia Flota", options=flotas_disp, required=True),
                "Configuración": st.column_config.SelectboxColumn("Formación", options=config_disp, required=True),
                "Cantidad": st.column_config.NumberColumn("N° Despachos Diarios", min_value=0, max_value=200, step=1)
            },
            num_rows="dynamic",
    ...
    if st.button("🚀 Ejecutar Simulación de Escenario", use_container_width=True, type="primary"):
        with st.spinner("Calculando integrales de energía para la oferta diseñada..."):
            RUTAS_PLAN = {
                "PU-LI": (0, 20, 1), "LI-PU": (20, 0, 2),
                "PU-SA": (0, 18, 1), "SA-PU": (18, 0, 2),
                "PU-BTO": (0, 14, 1), "BTO-PU": (14, 0, 2)
            }
            ...
            for idx, row in df_plan_edit.iterrows():
                ruta = row['Ruta']
                flota = row['Flota']
                es_doble = row['Configuración'] == "Doble"
                cant = row['Cantidad']
                
                if cant <= 0 or ruta not in RUTAS_PLAN: continue
                
                idx_ini, idx_fin, via = RUTAS_PLAN[ruta]
                km_ini = KM_ACUM[idx_ini]
                km_fin = KM_ACUM[idx_fin]
                
                if via == 1: est_idxs = range(idx_ini, idx_fin + 1)
                else: est_idxs = range(idx_ini, idx_fin - 1, -1)
                nodos_sint = [(0.0, KM_ACUM[i]) for i in est_idxs]
                
                trc, aux, reg, reo, neto, t_h = simular_tramo_termodinamico(
                    tipo_tren=flota, doble=es_doble, km_ini=km_ini, km_fin=km_fin, via_op=via, 
                    pct_trac=75, use_rm=False, use_pend=True, 
                    nodos=nodos_sint, pax_dict={}, pax_abordo=pax_promedio_viaje, 
                    v_consigna_override=None, maniobra=None
                )
                
                neto_real_unitario = max(0.0, trc + aux - (reg * 0.6))
                dist_km = abs(km_fin - km_ini)
                tren_km_unit = dist_km * (2 if es_doble else 1)
                
                kwh_total_fila = neto_real_unitario * cant
                km_total_fila = tren_km_unit * cant
                pax_total_fila = pax_promedio_viaje * cant
                
                tot_kwh += kwh_total_fila
                tot_km += km_total_fila
                tot_pax += pax_total_fila
                ...
                resultados_plan.append({
                    ...
                    "Viajes": cant,
                    "Consumo Unit. (Neto)": round(neto_real_unitario, 1),
                    "Consumo Total Fila (kWh)": round(kwh_total_fila, 0),
                    "Pax Transportados": pax_total_fila
                })
            
            if resultados_plan:
                df_res_plan = pd.DataFrame(resultados_plan)
                
                st.divider()
                st.markdown("### 📊 Resultados del Escenario Sintético")
                
                kp1, kp2, kp3, kp4 = st.columns(4)
                kp1.metric("⚡ Energía Total Diaria (Pantógrafo)", f"{tot_kwh:,.0f} kWh")
                kp2.metric("🧑‍🤝‍🧑 Pasajeros Proyectados", f"{tot_pax:,}")
                kp3.metric("📏 Producción Kilométrica", f"{tot_km:,.0f} Tren-km")
                ide_plan = tot_kwh / tot_km if tot_km > 0 else 0
                kp4.metric("💡 IDE del Escenario", f"{ide_plan:.3f} kWh/km")
                
                avg_power_kw = (tot_kwh / ETA_SER_RECTIFICADOR) / 18.0 
                demands_plan = {
                    'SER PO': avg_power_kw * 0.20,
                    'SER ES': avg_power_kw * 0.30,
                    'SER EB': avg_power_kw * 0.30,
                    'SER VA': avg_power_kw * 0.20
                }
                flujo_plan = calcular_flujo_ac_nodo(demands_plan)
                ac_loss_plan = flujo_plan['P_loss_kw'] * (1.15**2) * 18.0
                seat_kwh_plan = (tot_kwh / ETA_SER_RECTIFICADOR + ac_loss_plan) / 0.99
                
                st.info(f"**Auditoría de Requerimiento SEAT:**\n"
                        f"El consumo total proyectado en la SEAT de Alta Tensión será de **{seat_kwh_plan:,.0f} kWh**. "
                        f"En base a los perfiles de carga, aprox. el 18% de esta energía se consumirá en la hora de máxima demanda. "
                        f"Esto exigiría un pico de potencia sostenida de **{(seat_kwh_plan * 0.18):,.0f} kW** a la red de subestaciones, lo que equivale a un **{((seat_kwh_plan * 0.18) / SEAT_CAPACITY_KW)*100:.1f}% de la capacidad nominal de la SEAT (20 MVA)**.")

def render_ecodriving():
    st.subheader("🍃 Optimizador Cinemático (Eco-Driving)")
    st.markdown("Calcula el perfil de velocidad óptimo para un tramo, buscando el punto de deriva (coasting) que minimice el consumo de energía sin exceder un Tiempo Comercial Objetivo.")
    
    c_eco1, c_eco2, c_eco3 = st.columns(3)
    with c_eco1:
        est_ini = st.selectbox("Estación Origen", ESTACIONES, index=0)
        est_fin = st.selectbox("Estación Destino", ESTACIONES, index=12)  # Quilpué por defecto
    with c_eco2:
        tipo_t = st.selectbox("Tipo de Tren (Eco)", ["XT-100", "XT-M", "SFE"])
        trac_p = st.slider("Esfuerzo de Tracción (%)", 50, 100, 100, 5)
    with c_eco3:
        t_obj_min = st.number_input("Tiempo Comercial Objetivo (minutos)", value=25.0, step=0.5)
        
    st.info("⚠️ **Nota de Calibración:** Los resultados de este módulo son teóricos (Línea Base). Para el reporte energético de la empresa, la métrica de **facturación** eléctrica se debe priorizar siempre por encima de las lecturas PRMTE o de subestación (SEAT).")

    if st.button("🚀 Ejecutar Optimización Eco-Driving", type="primary"):
        idx_i = ESTACIONES.index(est_ini)
        idx_f = ESTACIONES.index(est_fin)
        
        if idx_i == idx_f:
            st.warning("El origen y destino deben ser distintos.")
        else:
            via_sel = 1 if idx_i < idx_f else 2
            km_i = KM_ACUM[idx_i]
            km_f = KM_ACUM[idx_f]
            t_obj_s = t_obj_min * 60.0
            
            with st.spinner("Buscando punto óptimo de deriva mediante búsqueda binaria..."):
                df_eco = calcular_perfil_eco_driving(tipo_t, km_i, km_f, via_sel, trac_p, t_obj_s)
                
            if df_eco.empty:
                st.error(f"❌ Imposible cumplir el trayecto {est_ini} -> {est_fin} en {t_obj_min} minutos. El tiempo objetivo es demasiado estricto para la física del tren.")
            else:
                st.success("✅ Perfil de conducción óptimo calculado.")
                
                # Plotly Chart
                fig_eco = go.Figure()
                fig_eco.add_trace(go.Scatter(x=df_eco['Distancia_m'], y=df_eco['Velocidad_kmh'], 
                                             mode='lines', name='Velocidad', line=dict(color='#1565C0', width=3)))
                
                # Marcar zonas de estado
                df_eco['cambio_estado'] = df_eco['Estado'] != df_eco['Estado'].shift()
                puntos_cambio = df_eco[df_eco['cambio_estado']]
                
                for _, row in puntos_cambio.iterrows():
                    color_v = 'green' if row['Estado'] == 'COAST' else 'red' if row['Estado'] == 'BRAKE' else 'blue'
                    fig_eco.add_vline(x=row['Distancia_m'], line_dash="dash", line_color=color_v, annotation_text=row['Estado'])
                    
                fig_eco.update_layout(title="Perfil Espacial de Eco-Conducción", xaxis_title="Distancia (m)", yaxis_title="Velocidad (km/h)")
                st.plotly_chart(fig_eco, use_container_width=True)
                
                st.markdown("### 📋 Tabla de Resultados (Puntos de Inflexión y Salida)")
                
                # Mostrar tabla con Fechas y Horas en columnas separadas (Mandato de usuario)
                # Resumir un poco para no mostrar miles de filas en pantalla
                df_mostrar = df_eco[df_eco.index % 10 == 0].copy()
                st.dataframe(df_mostrar, use_container_width=True)
                
                csv_eco = df_eco.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="📥 Descargar Perfil Completo (CSV)",
                    data=csv_eco,
                    file_name=f'Perfil_EcoDriving_{est_ini}_{est_fin}.csv',
                    mime='text/csv'
                )
...
if df_all.empty and df_px.empty:
    ...
    tab_mapa, tab_datos, tab_vacios, tab_planificador, tab_ecodriving = st.tabs(["🗺️ Mapa Operativo", "📋 Reporte Pasajeros (Directo)", "🚉 Maniobras en Vacío", "🔮 Planificador de Escenarios", "🍃 Optimizador Eco-Driving"])
    ...
    with tab_mapa:
        if df_all.empty:
            ...
            st.plotly_chart(draw_diagram(df_act, {k: max(0.0, v) for k, v in ser_accum_1.items()}, seat_accum_1, hora_s1, "", active_sers), use_container_width=True)
