        ...
        try:
            ...
            for sn in xl.sheet_names:
                ...
                if any(k in sn_up for k in ['PRMTE', 'MEDIDAS']):
                    df_pd = pd.read_excel(f, sheet_name=sn, header=None)
                    h_idx = next((i for i in range(len(df_pd)) if 'AÑO' in str(df_pd.iloc[i]).upper()), None)
...
with tabs[0]:
    if not df_ops.empty:
        ...
        fig.add_trace(go.Bar(x=df_ops['Fecha'], y=df_ops['Odómetro [km]']/1000, name='Odómetro (kkm)'), secondary_y=False)
        fig.add_trace(go.Scatter(x=df_ops['Fecha'], y=df_ops['UMR [%]'], name='UMR (%)', mode='lines+markers'), secondary_y=True)
...
with tabs[1]:
    if not df_ops.empty: st.dataframe(df_ops.style.format({'Odómetro [km]': "{:,.1f}", 'UMR [%]': "{:.2f}%", 'IDE (kWh/km)': "{:.4f}"}))

with tabs[2]:
    if not df_tr.empty:
        st.write("### Kilometraje Diario")
        st.dataframe(df_tr.pivot_table(index="Tren", columns="Día", values="Valor", aggfunc='sum').fillna(0).style.format("{:,.1f}"))
...
with tabs[4]:
    if all_comp_full:
        df_c = pd.DataFrame(all_comp_full).pivot_table(index="Hora", columns=pd.DataFrame(all_comp_full)['Fecha'].dt.year, values="Consumo Horario [kWh]", aggfunc='median').fillna(0)
        st.line_chart(df_c)
...
with tabs[7]:
    ...
    def mostrar_thdr(df, titulo, em):
        st.subheader(f"{em} {titulo}")
        if not df.empty:
            df['H_Prog'] = df['Min_Prog'].apply(lambda x: format_hms(x))
            df['H_Real'] = df['Min_S_Real'].apply(lambda x: format_hms(x))
            st.dataframe(df[['Fecha_Op', 'Servicio', 'H_Prog', 'H_Real', 'Motriz 1', 'Tipo_Rec', 'Tren-Km']])
    mostrar_thdr(df_thdr_v1, "Vía 1", "🟢")
    mostrar_thdr(df_thdr_v2, "Vía 2", "🔵")

st.sidebar.download_button("📥 Reporte Completo", to_excel_consolidado(df_ops, df_tr, df_tr_acum, df_seat, df_p_d, pd.DataFrame(all_prmte_15), pd.DataFrame(all_fact_h), df_f_d), "Reporte_SGE.xlsx")