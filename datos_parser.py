# datos_parser.py
# Módulo ETL (Extract, Transform, Load) para THDR, Planilla Maestra y Pasajeros.
...
from config import ESTACIONES, EC, KM_ACUM, KM_TOTAL, N_EST, PAX_COLS, EST_LATS, EST_LONS

# =============================================================================
# FUNCIONES DE TIEMPO Y PARSEO
# =============================================================================
...
def parse_excel_date(val):
    ...
    if not v_str or v_str.lower() in ['nan', 'none', 'fecha', 'date', 'nat']: 
        return None
    ...
    if v_str.isdigit():
        ...
        if 40000 <= v_int <= 60000:
            try: 
                return (date(1899, 12, 30) + timedelta(days=v_int)).strftime('%Y-%m-%d')
            except: 
                pass
...
def clean_pax_number(x):
    ...
    try: 
        return int(s) if s and s != 'nan' else 0
    except: 
        return 0
...
# =============================================================================
# LECTORES Y PARSERS PRINCIPALES
# =============================================================================

def procesar_thdr(data, fname, via_param=1):
    """Lector Histórico de THDR expandido para evitar cortes y extraer num_servicio."""
    try:
        ext = fname.lower()
        if ext.endswith('.csv'):
            try: 
        ...
        if raw is None or raw.empty: 
            return pd.DataFrame(), f"Archivo vacío o ilegible: {fname}"
        if raw.shape[0] < 6: 
            return pd.DataFrame(), f"Archivo muy corto: {fname}"
        ...
        if df.empty: 
            return pd.DataFrame(), f"Sin filas tras limpiar: {fname}"
        ...
        if not sal_cols or not lle_cols: 
            return pd.DataFrame(), "Faltan columnas de hora."
        ...
        # EXTRACCIÓN CRÍTICA RESTAURADA (num_servicio)
...
def parsear_planilla_maestra(data, fname):
    """Extrae Viajes y Flota con regla Anti-Contagio (V117)"""
    try:
        ext = fname.lower()
        dfs = {}
        if ext.endswith('.csv'):
...
# =============================================================================
# CARGADORES DE STREAMLIT PARA LA UI
# =============================================================================
def leer(files): 
