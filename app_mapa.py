# app_mapa.py
# Controlador UI, Orquestador y Motor Físico del Simulador MERVAL v117
# Arquitectura Unificada: Física + ETL + UI
        ...
        if 40000 <= v_int <= 60000:
            ...
            try:
                ...
                if 1 <= d <= 31 and 1 <= m <= 12: 
                    return f"{2000+y if y<100 else y:04d}-{m:02d}-{d:02d}"
            except: pass
...
# =============================================================================
# 3. MOTOR FÍSICO Y CINEMÁTICO (MEJORADO CON JERK CONTROL EN 50126)
# =============================================================================
...
def simular_tramo_termodinamico(tipo_tren, doble, km_ini, km_fin, via_op, pct_trac, use_rm, use_pend, nodos=None, pax_dict=None, pax_abordo=0, v_consigna_override=None, maniobra=None, estacion_anio="primavera", t_ini_mins=0.0, es_vacio=False):
    ...
    for i in range(len(paradas_km)-1):
        ...
        dist_recorrida = 0.0
        v_ms = 0.0
        a_prev = 0.0 # Mejora: Control de Jerk
        estado_marcha = "ACCEL"
        ...
        while dist_recorrida < dist_total_tramo:
            ...
            # MEJORA APLICADA: Control de Jerk para suavidad (EN 50126)
            jerk_limit = 0.8 * dt
            if a_net_target > a_prev + jerk_limit:
                a_net = a_prev + jerk_limit
            elif a_net_target < a_prev - jerk_limit:
                a_net = a_prev - jerk_limit
            else:
                a_net = a_net_target
            a_prev = a_net
...
def distribuir_potencia_sers_kw(p_kw, km_punto, active_sers):
    ...
    for i in range(len(sers_sorted)-1):
        if sers_sorted[i][0] <= km_punto <= sers_sorted[i+1][0]:
            dist_total = sers_sorted[i+1][0] - sers_sorted[i][0]
            return {sers_sorted[i][1]: p_kw * ((sers_sorted[i+1][0] - km_punto) / dist_total), sers_sorted[i+1][1]: p_kw * ((km_punto - sers_sorted[i][0]) / dist_total)}
...
# =============================================================================
# 8. MÉTODOS ELÉCTRICOS, EXTRACCIÓN Y DATOS (PARSERS)
# =============================================================================
...
def draw_diagram(df_act_plot, ser_accum_plot, seat_accum_plot, hora_str, titulo_extra="", active_sers_list=SER_DATA, gap_vias=200):
    ...
    for idx, row in df_act_plot.iterrows():
        ...
        fig.add_trace(go.Scatter(
            x=[xp], y=[y_ln], mode='markers', 
            marker=dict(size=r_c*2, color=color, line=dict(color='black', width=2)), 
            hovertext=row.get('tooltip', ''), hovertemplate='%{hovertext}<extra></extra>', showlegend=False
        ))
        ...
        if sep_s: 
            fig.add_annotation(x=xp, y=y_ln + dy_sep, text=f"<b>{sep_s}</b>", showarrow=False, font=dict(size=12, color='#111'))
